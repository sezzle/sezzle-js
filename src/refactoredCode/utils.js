...
const utils = {
  ...
  async getCountryCodeFromIP() {
    let response = await httpRequestWrapper('GET', state.apiEndpoints.countryFromIPRequestURL);
  ...
  async loadCSS() {
    ...
    if (!cssVersion) {
      let response = await httpRequestWrapper('GET', state.apiEndpoints.cssForMerchantURL);
      response = JSON.parse(response);
  ...
  startObserve(element, callback) {
    ...
    observer.observe(element, state.mutationObserverConfig);
    return observer;
  },
  /**
  ...
  getFormattedPrice(element, configGroupIndex, priceText) {
    ...
    const sezzleInstallmentPrice = (price / state.numberOfPayments).toFixed(2);
    // format the string
  ...
  insertStoreCSSClassInElement(element) {
    element.className += ` sezzle-${state.merchantID}`;
    return element;
  },
  /**
  ...
  addCSSAlignment(element, configGroupIndex) {
    ...
    if (matchMedia && state.configGroups[configGroupIndex].alignmentSwitchMinWidth && state.configGroups[configGroupIndex].alignmentSwitchType) {
      const queryString = `(min-width: ${state.configGroups[configGroupIndex].alignmentSwitchMinWidth}px)`;
