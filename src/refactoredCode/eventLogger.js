...
const logEvent = (eventName, configGroupIndex) => {
    if (!state.noTracking) {
        ...
        if (sezzleFrame) {
            let viewport = { width: null, height: null };
            viewport.width = screen && screen.width ? screen.width : console.log("Cant fetch viewpoer width");
            viewport.height = screen && screen.height ? screen.height : console.log("Cant fetch viewpoer height");
