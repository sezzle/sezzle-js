...
const getElementToRender = (element, index = 0) => {
  ...
  if (state.configGroups[index].rendertopath !== null) {
    const path = breakXPath(state.configGroups[index].rendertopath);
  ...
  return toRenderElement || element.parentElement; // return the element's parent if toRenderElement is null
};
  ...
  if (targets) {
    targets.forEach((target) => {
      if (typeof (target.relatedPath) === 'string'
        ...
        const elements = getElementsByXPath(
...
const sezzleWidgetCheckInterval = () => {
  ...
  state.configGroups.forEach((configGroup, index) => {
    ...
    const elements = getElementsByXPath(configGroup.xpath);
    elements.forEach((e) => {
...
export const initWidget = () => {
  ...
  state.configGroups.forEach((configGroup, index) => {
    if (configGroup.hasPriceClassElement) {
      const sz = renderAwesomeSezzle(configGroup.priceElements[0], configGroup.renderElements[0], 0, index);
