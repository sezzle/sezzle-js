...
const disableBodyScroll = (disable) => {
		...
		if (disable) {
			// Cross-browser
			scrollDistance = pageYOffset || (document.documentElement.clientHeight ? document.documentElement.scrollTop : document.body.scrollTop) || 0;
...
const closeSezzleModalHandler = () => {
		...
		const sezzleModal = document.getElementsByClassName('sezzle-modal')[0];
...
const renderAPModal = () => {
	let modalNodeAP;
	modalNodeAP = document.createElement('div');
...
const renderQPModal = () => {
	let modalNodeQP;
	modalNodeQP = document.createElement('div');
...
const renderAffirmModal = () => {
	let modalNodeAffirm;
	modalNodeAffirm = document.createElement('div');
...
export const addClickEventForModal = (sezzleElement, configGroupIndex) => {
		let modalLinks = sezzleElement.getElementsByClassName('sezzle-modal-link');
			...
			modalLink.addEventListener('click', function (event) {
				if (!event.target.classList.contains('no-sezzle-info')) {
					...
					if (modalNode) {
						...
						let modals = modalNode.getElementsByClassName('sezzle-modal');
						if (modals.length) {
		...
		// for AfterPay
		let apModalLinks = sezzleElement.getElementsByClassName('ap-modal-info-link');
		...
		// for QuadPay
		let qpModalLinks = sezzleElement.getElementsByClassName('quadpay-modal-info-link');
